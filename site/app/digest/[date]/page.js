...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: {
      canonical: `${siteConfig.siteUrl}/digest/${date}`,
    },
    openGraph: {
...
export default async function DigestPage({ params }) {
  ...
  const articleSchema = stories.map((story) => ({
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: story.title,
    description: story.whyItMatters || story.body.join(' ').slice(0, 200),
    url: story.url || siteConfig.siteUrl,
    datePublished: date,
    publisher: {
      '@type': 'Organization',
